...
export function AudioArea() {
  ...
  const [audioStates, setAudioStates] = useState<
    Record<string, AudioPlayerState>
  >({});
    ...
    try {
      ...
      setTimeout(() => {
        toast({
          ...
          duration: 3000,
          className:
            "bg-toastBackgroundColor border border-borderColorPrimary text-foreground",
        });
      }, 0);
    } catch (err) {
    ...
    (modelId: string) => {
      ...
      if (newLikedState) {
        ...
        const likedItems = useLikedMediaStore
          .getState()
          .getLikedMediaByType("audio");
        const likedItem = likedItems.find(
          (item) => item.modelId === modelId && item.url === response.audioUrl
        );
        if (likedItem) {
              ...
              {isLoading ? "Generating..." : "Generate"}
                          ...
                          <div className="border rounded-lg p-4">
                            <div className="space-y-4">
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  <Button
                                    ...
                                    onClick={() =>
                                      handlePlayPause(response.modelId)
                                    }
                                  >
                                    {audioStates[response.modelId]
                                      ?.isPlaying ? (
                              ...
                              <div className="space-y-2">
                                <div className="flex justify-between text-sm text-muted-foreground">
                                  ...
                                  <span>
                                    {formatTime(
                                      audioStates[response.modelId]?.duration ||
                                        0
                                    )}
                                  </span>
                                </div>
                                ...
                                <Slider
                                  ...
                                  step={0.1}
                                  onValueChange={(value) =>
                                    handleSliderChange(response.modelId, value)
                                  }
                            ...
                            <audio
                              ...
                              onTimeUpdate={() =>
                                handleTimeUpdate(response.modelId)
                              }
                              onLoadedMetadata={() =>
                                handleLoadedMetadata(response.modelId)
                              }
                              onEnded={() => {
                                setCurrentlyPlaying(null);
                                setAudioStates((prev) => ({
