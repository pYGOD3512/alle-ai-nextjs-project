...
import { userGuides, apiReference } from "@/lib/constants/docs";
...
export const SidebarNav = () => {
  ...
  const isActive = (path: string, id?: string) => {
    ...
    const isChatPage = pathname === "/docs/api-reference/chat";
    const isIntroduction = pathname === "/docs/api-reference/introduction";

    if (isChatPage) {
      return pathname === `${path}/id`;
    }
    if (isIntroduction) {
    ...
    // Default behavior for non-chat pages
    return pathname === path;
  };

  const handleReferenceClick = (sectionId: string, url: string) => {
    ...
    if (pathname !== url) {
      ...
      if (element) {
        ...
        window.history.replaceState(null, "", `${url}#${sectionId}`);
      }
    } else {
      const element = document.getElementById(sectionId);
      if (element) {
                        ...
                        {section.sections.map((subsection, index) => (
                          <div key={subsection.id}>
                            <Link
                              ...
                              className={`group flex items-center w-3/4 rounded-md p-2 text-sm transition-all duration-200
